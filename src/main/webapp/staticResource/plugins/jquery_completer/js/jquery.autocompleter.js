...
;(function ($, window) {
     ...
     */
    function _search(query, source, limit,labelName) {
        var response = [];
            ...
            for (var i = 0; i < 2; i++) {
                for (var item in source) {
                    if (response.length < limit) {
                        switch (i) {
                            case 0:
                                if (source[item][labelName].toUpperCase().search(query) === 0) {
                            ...
                            case 1:
                                if (source[item][labelName].toUpperCase().search(query) !== -1) {
                                    response.push(source[item]);
                                    delete source[item];
                                }
                            break;
    ...
    function _launch(data) {
        ...
        if (!data.empty && data.query.length === 0) {
            ...
            if (typeof data.source === 'object') {
                ...
                // Local search
                var search = _search(data.query, _clone(data.source), data.limit,data.labelName);
                if (search.length) {
    ...
    function _buildList(list, data) {
        ...
        for (var item = 0, count = list.length; item < count; item++) {
            ...
            if (data.selectFirst && item === 0 && !data.changeWhenSelect) {
                /*var label=list[item][data.labelName];
                var value=list[item][data.valueName];
                $("#"+data.valId).val(value);
                $("#"+data.selId).val(label);
                $("#"+data.selVal).val(value);*/
                classes.push("autocompleter-item-selected");
            }

            var highlightReg = new RegExp(data.query, "gi");
            var label = (data.customLabel && list[item][data.customLabel]) ? list[item][data.customLabel] : list[item][data.valueName];

            var clear = label;

            var value = (data.customValue && list[item][data.customValue]) ? list[item][data.customValue] : list[item][data.valueName];

            if(data.highlightMatches){
                label=label.replace(highlightReg, "<strong>$&</strong>")
            }
        ...
        if (list.length && data.hint) {
            var hint = ( list[0][data.labelName].substr(0, data.query.length).toUpperCase() === data.query.toUpperCase() ) ? list[0][data.labelName] : false;
            if (hint && (data.query !== list[0][data.labelName])) {
