package cn.com.edzleft.controller.captial.certificate;
...
import cn.com.edzleft.service.captial.certificate.CapCertificateService;
...
public class CapCertificateController {
	
	@Autowired
	private CapCertificateService capCertificateService;
	/**
	 * ukey与证书
    ...
    public DataGridJson ukeyList(HttpSession session,Integer pageNumber,Integer pageSize,String issuedPeople,String ukeyStatus) {
        ...
        userPage = capCertificateService.getUkeyEntityListByConditions(userPage);
	...
	public Map<String, Object> getByUserId(HttpSession session) {
		...
		SessionInfo sessionInfo = (SessionInfo) session.getAttribute("sessionInfo");
		Account sessionAccount=sessionInfo.getAdmin();
		map =capCertificateService.getByUserId(sessionAccount.getUserId());
		return map;
	}
