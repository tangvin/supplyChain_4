package cn.com.edzleft.controller.procurement.setting;
...
import cn.com.edzleft.service.procurement.setting.ProSettingService;

@Controller
@RequestMapping("/proSetting")
public class ProSettingController {
	...
	public Map<String, Object> getSettingByUserId(HttpSession session) {
		...
		Settings setting = proSettingService.querySettingByUserId(sessionAccount.getUserId());
	...
	public Object update(Integer shortMessage,Integer email,HttpSession session) {
		...
		Integer res=proSettingService.update(setting);
		if (res>0) {
	...
	public Map<String, Object> insert(HttpSession session) {
		...
		Account sessionAccount = sessionInfo.getAdmin();
		Integer userId = sessionAccount.getUserId();
		Integer count=proSettingService.getCount(userId);
		if (count == 0) {
			...
			setting.setEmail(email);
			setting.setUserId(userId);
			proSettingService.insert(setting);
